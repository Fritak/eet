...
class Certificate
{
    ...
    {
        if (empty($config['certificate']['certificate']) && !file_exists($config['certificate']['path']))
        {
        ...
        $certs = [];
        $pkcs12 = !empty($config['certificate']['certificate'])? $config['certificate']['certificate'] : file_get_contents($config['certificate']['path']);
